...
export class EmployredinComponent implements OnInit {
            ...
            if (res.success) {
              Swal.fire({
                ...
                let update = {
                  ...
                  newdata: this.Employer.value.Employer_in_name,
                  tablename: 'user_education_occupations',
                  coulemnname: 'user_employed_In',
                };
  ...
  update(data: any) {
    ...
    this.action = 'Update';
    this.ApiParameter.fetchdata('employer_in', { projection: ['*'] }).subscribe(
      (res: any) => {
        if (res.success) {
          // this.tabledata = res['data'];
