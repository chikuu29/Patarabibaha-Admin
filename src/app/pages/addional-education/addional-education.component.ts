...
export class AddionalEducationComponent implements OnInit {
  ...
  getAllData(
    ...
    // this.ApiParameter.fetchdata('additional_education', { "projection": ["*"] },offset,10).subscribe((res: any) => {
  ...
  update(data: any) {
    ...
    this.ApiParameter.fetchdata('additional_education', {
      ...
      if (res.success) {
        ...
        this.editedcast = res['data'][0].additional_education_name;
      }
    });
  }
  deleted(data: any) {
    this.blockUI.start('Deleting...');
    this.ApiParameter.deletedata('additional_education', {
      whereConditions: { id: data },
    }).subscribe((res: any) => {
      this.blockUI.stop();
      if (res.success) {
