...
export class GraphDataViewComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.apiparameter.fetchDataFormQuery(query_customer).subscribe((res: any) => {
      console.log("res", res);
    ...
    let agricultureCaseQuery = "SELECT user_membership_plan_type,COUNT(*) AS count FROM user_info   GROUP BY user_membership_plan_type;"
    this.apiparameter.fetchDataFormQuery(agricultureCaseQuery).subscribe((res: any) => {
      console.log("res", res);
      ...
      if (res.success) {
        ...
        this.freeMemberCount = _.filter(res['data'], { 'user_membership_plan_type': "Free" })[0].count
        this.paidMemberCount = _.filter(res['data'], (i) => { return i.user_membership_plan_type != 'Free' })[0].count
...
this.basicDataForCustomer={
  ...
  createPaiChartForAgricultureCase(data: any) {
    ...
    this.paiChartCasedata = {
      ...
      datasets: [
        {
          data: [this.paidMemberCount, this.acceptedCaseCounte, this.freeMemberCount],
          backgroundColor: [documentStyle.getPropertyValue('--yellow-400'), documentStyle.getPropertyValue('--yellow-500'), documentStyle.getPropertyValue('--green-500')],
          hoverBackgroundColor: [documentStyle.getPropertyValue('--yellow-400'), documentStyle.getPropertyValue('--yellow-400'), documentStyle.getPropertyValue('--green-400')]
        }
      ]
    };
