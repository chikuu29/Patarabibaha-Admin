...
@Component({
  ...
  styleUrls: ['./emailvalidation.component.scss'],
})
export class EmailvalidationComponent implements OnInit {
  ...
  fatch(start: number, limit: number, loadSpecificData: boolean = false, search_text?: any) {
    ...
    let Quary =
      `select * ,COUNT(*) OVER () AS total_count from user_info as a left join auth_user as b on a.user_id = b.auth_ID WHERE a.user_mail_varification = 0
      LIMIT ${limit} OFFSET ${start}`;
      if (loadSpecificData) {
        Quary = `select * ,COUNT(*) OVER () AS total_count
        from user_info as a left join auth_user as b
        on a.user_id = b.auth_ID
        WHERE a.user_mail_varification = 0
