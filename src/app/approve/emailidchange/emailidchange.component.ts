...
@Component({
  ...
  styleUrls: ['./emailidchange.component.scss']
})
export class EmailidchangeComponent implements OnInit {
  ...
  getMobileApprove(
    ...
    if (loadSpecificData) {
      query = `
      SELECT email_id_update.* ,user_info.user_full_name,user_info.user_phone_no,user_info.user_email,
      COUNT(*) OVER () AS total_count
      FROM email_id_update
      JOIN user_info ON email_id_update.user_id = user_info.user_id
             WHERE user_info.user_id = '${search_text}'
                   ...
                   ORDER BY email_id_update.user_id ASC, email_id_update.id ASC
      ...
      FROM email_id_update
      JOIN user_info ON email_id_update.user_ID = user_info.user_id
      where email_id_update.change_states = 0
        ORDER BY email_id_update.change_states ASC, email_id_update.id ASC
            LIMIT ${limit} OFFSET ${start};
        `;
    }
  ...
  update(data: any, newgender: any, oldgender: any) {
    Swal.fire({
      icon: 'question',
      ...
      if (ress.isConfirmed) {
        let query = ` SELECT * FROM auth_user WHERE auth_email = '${newgender}' `;

        this.ApiParameter.fetchDataFormQuery(query).subscribe((res: any) => {
          console.log(res);
          if (res.success && res['data'].length > 0) {
            Swal.fire({
              text: 'That Email Id already exists. For '+res['data'][0].auth_ID
            });
  ...
  //   Swal.fire({
  //     icon: 'question',
  //     text: 'Do you want to Change Gender Of ' + data,
  //     showCancelButton: true,
  //   }).then((ress: any) => {
  //     if (ress.isConfirmed) {
  //       let updateData = {
  ...
  //               whereConditions: { auth_ID: data },
  //             };
  //             this.ApiParameter.updatedata('auth_user', updateData).subscribe(
  //               (resdata: any) => {
  //                 if (resdata.success) {
  //                   let updateData1 = {
  //                     data: {
