...
export class SalapprovalComponent implements OnInit {
    ...
    if (loadSpecificData) {
      ...
      JOIN user_info ON use_payment_slip_upload.user_ID = user_info.user_id
             WHERE user_info.user_id = '${search_text}'
                   ...
                   ORDER BY use_payment_slip_upload.status ASC, use_payment_slip_upload.id DESC
  ...
  addimg() {
    ...
    this.ApiService.paymentSlipUplode(param).subscribe((res: any) => {
      if (res.success) {
        Swal.fire({
          icon: 'success',
          title: 'Uplode photo',
