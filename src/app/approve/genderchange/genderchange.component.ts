...
@Component({
  selector: 'app-genderchange',
  templateUrl: './genderchange.component.html',
  styleUrls: ['./genderchange.component.scss']
})
export class GenderchangeComponent implements OnInit {
  ...
  currentFunction: string = 'getGenderApprove';
  tableData: any;
  filterText: any;
  ...
  ngOnInit(): void {
    ...
    this.getGenderApprove(0, this.totalFetchrecord);
  }
  ...
  getGenderApprove(
    ...
    if (loadSpecificData) {
      query = `
      SELECT gender_update.* ,user_info.user_full_name,user_info.user_phone_no,user_info.user_email,
      COUNT(*) OVER () AS total_count
      FROM gender_update
      JOIN user_info ON gender_update.user_id = user_info.user_id
             WHERE user_info.user_id = '${search_text}'
                   ...
                   ORDER BY gender_update.user_id ASC, gender_update.id ASC
      ...
      FROM gender_update
      JOIN user_info ON gender_update.user_ID = user_info.user_id
      where gender_update.change_states = 0
        ORDER BY gender_update.change_states ASC, gender_update.id ASC
            LIMIT ${limit} OFFSET ${start};
        `;
    }
        ...
        this.ApiParameter.updatedata('user_info', updateData).subscribe(
          (resdata: any) => {
            if (resdata.success) {
              ...
              this.ApiParameter.updatedata('gender_update', updateData1).subscribe(
                (resdata: any) => {
                  if (resdata.success) {
                    let params = {
                      "user_id": data,
