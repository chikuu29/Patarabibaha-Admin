...
@Component({
  selector: 'app-whatsappnochange',
  templateUrl: './whatsappnochange.component.html',
  styleUrls: ['./whatsappnochange.component.scss']
})
export class WhatsappnochangeComponent implements OnInit {
  ...
  getMobileApprove(
    ...
    if (loadSpecificData) {
      query = `
      SELECT whatsapp_no_update.* ,user_info.user_full_name,user_info.user_phone_no,user_info.user_email,
      COUNT(*) OVER () AS total_count
      FROM whatsapp_no_update
      JOIN user_info ON whatsapp_no_update.user_id = user_info.user_id
             WHERE user_info.user_id = '${search_text}'
                   ...
                   ORDER BY whatsapp_no_update.user_id ASC, whatsapp_no_update.id ASC
      ...
      FROM whatsapp_no_update
      JOIN user_info ON whatsapp_no_update.user_ID = user_info.user_id
      where whatsapp_no_update.change_states = 0
        ORDER BY whatsapp_no_update.change_states ASC, whatsapp_no_update.id ASC
            LIMIT ${limit} OFFSET ${start};
        `;
    }
  ...
  update(data: any, newgender: any, oldgender: any ,ccode:any) {
    Swal.fire({
      icon: 'question',
      text: 'Do you want to Change Whatsapp no Of ' + data,
      showCancelButton: true,
    }).then((ress: any) => {
      if (ress.isConfirmed) {
        let query = ` SELECT * FROM user_info WHERE user_whatsapp_no = '${newgender}' `;

        this.ApiParameter.fetchDataFormQuery(query).subscribe((res: any) => {
          console.log(res);
          if (res.success && res['data'].length > 0) {
            Swal.fire({
              text: 'That Whatsapp no already exists. For '+res['data'][0].user_id
            });

          } else {
            let updateData = {
              data: {
                user_whatsapp_no: newgender,
                whats_app_c_code: ccode
              },
